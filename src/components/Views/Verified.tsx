...
import { ImageBackground, StyleSheet, View, Image, Text, SafeAreaView, BackHandler} from 'react-native';
...
const Verified: React.FC<VerifiedProps> = ({ route, navigation }: VerifiedProps) => {
  const { payload, check } = route.params;
  
  console.log({payload, check});
  const name = payload[0].name;
  const lastName = payload[0].lastname;
  const id = payload[0].id;

  const currentDate = new Date();
  const formattedDate = moment(currentDate).format('DD-MM-20YY');
      ...
      <SafeAreaView style={styles.container}>
        <ImageBackground
          ...
          {result && result.statusCode !== 200 ? (
              ...
              <View style={styles.textContainer}>
                <Text style={[styles.welcome, {fontSize:scaleFontSize(35)}]}>
                  {check === 'in' ? 'Welcome!' : 'Farewell!'}
                </Text>
          ...
          {result && result.message === "You are already checked in" && (
